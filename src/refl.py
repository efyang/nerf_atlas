...
def load(args, refl_kind:str, space_kind:str, latent_size:int):
  ...
  elif refl_kind == "multfvrview": kwargs["out_features"] = 6
...
class MultFVRView(Reflectance):
  def __init__(
    ...
    **kwargs,
    ...
    in_size = view_dims
    self.mlp = SkipConnMLP(
      in_size=in_size, out=2, latent_size=self.latent_size,
      # enc=FourierEncoder(input_dims=in_size),
